...
outdir <- '../outputs/dexseq_covariates/FECDvC'
...
allSamples$condition[allSamples$condition != 'control'] <- 'FECD' # group both Exp+ and Exp- under FECD label
...
pdf(file.path(outdir, 'TCF4_FECDvC_cov.pdf', fsep = '/'))
plotDEXSeq( dxr1, "ENSG00000196628.20", expression=FALSE, norCounts=TRUE,
   legend=TRUE)
dev.off() 

#Visualising full results in HTML
DEXSeqHTML(dxr1, FDR=0.05, color=c("#FF000080", "#0000FF80"), path = file.path(outdir, 'DEXSeqReport_FECDvC_cov', fsep = '/'))
...
fwrite(dxr1.df.rmNA.pord, file.path(outdir, "DEXSeq_FECDvC_all.csv", fsep = '/'))

#exporting just sig events
rdxr1.df.rmNA.pord <- dxr1.df.rmNA.pord %>% filter(padj<=0.05)
head(rdxr1.df.rmNA.pord)
fwrite(rdxr1.df.rmNA.pord, file.path(outdir, "DEXSeq_FECDvC_sig.csv", fsep = '/'))