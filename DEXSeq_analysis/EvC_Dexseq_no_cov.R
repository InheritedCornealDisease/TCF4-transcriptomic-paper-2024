...
allSamples <- read.csv("../DEXseq_fromNAS/batch3counts/A160vC_sample.csv")
sampleTable <- subset(allSamples, allSamples$condition %in% c('E_FECD', 'control'))

dxd=DEXSeqDataSetFromHTSeq(
  countFiles, sampleData = sampleTable, design= ~ sample + exon + condition:exon, flattenedfile = flattenedFile) 
...
# plot TCF4
pdf(file.path(outdir, 'TCF4_EvC.pdf', fsep = '/'))
plotDEXSeq( dxr1, "ENSG00000196628.20", expression=FALSE, norCounts=TRUE,
   legend=TRUE)
dev.off() 

#Visualising full results in HTML
DEXSeqHTML(dxr1, FDR=0.05, color=c("#FF000080", "#0000FF80"), path = file.path(outdir, 'DEXSeqReport_EvC', fsep = '/'))
